...
lib_summary <- function(..., sizes = FALSE) {
  if (!is.logical(sizes)) {
    cli::cli_abort("{.var sizes} should be a {.cls {class(TRUE)}} value, not {.obj_type_friendly {sizes}}")
  }
  
  lib() |> 
    calculate_sizes(calc = sizes) |> 
    dplyr::group_by(...) |> 
    dplyr::summarise(
      n = dplyr::n(),
...
calculate_sizes <- function(df, calc) {
  if (!isTRUE(calc)) return(df)
