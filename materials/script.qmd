...
Turn this into a function that can summarise on any variable

```{r}
sw_summary <- function(var) {
  starwars |> 
    group_by(var) |> 
    count()
}
...
```{r}
use_package("dplyr")
use_r("summarize_pkgs")
```

```{r}
summarize_pkgs <- function(by) {
  pkgs <- dplyr::as_tibble(utils::installed.packages())
...
summarize_pkgs <- function(by) {
  ...
  pkgs <- dplyr::mutate(
    pkgs,
    size = map_dbl(fs::path(LibPath, Package), \(x) {
      sum(fs::file_size(fs::dir_ls(x, recurse = TRUE)))
    })
  )
...
summarize_pkgs <- function(by) {
  ...
  pkgs <- dplyr::mutate(
    pkgs,
    size = map_dbl(fs::path(.data$LibPath, Package), \(x) {
      sum(fs::file_size(fs::dir_ls(x, recurse = TRUE)))
    })
  )
...
summarize_pkgs <- function(...) {
  ...
  dplyr::group_by(pkgs, ...) |>
    dplyr::summarise(n = dplyr::n(), size = sum(.data$size))
}
```
